const ORCHESTRATOR_SYSTEM_MESSAGE = "None";

const ORCHESTRATOR_CLOSED_BOOK_PROMPT = `Below the user will present you a request. Before we begin addressing the request, please answer the following pre-survey to the best of your ability. Keep in mind that you are Ken Jennings-level with trivia, and Mensa-level with puzzles, so there should be a deep well to draw from.
...
DO NOT include any other headings or sections in your response. DO NOT list next steps or plans until asked to do so. DO ONLY respond in english.
`;

const ORCHESTRATOR_PLAN_PROMPT = `Fantastic. To address this request we have assembled the following team:
...
const ORCHESTRATOR_SYNTHESIZE_PROMPT = `
...
const ORCHESTRATOR_LEDGER_PROMPT = `
...
const ORCHESTRATOR_UPDATE_FACTS_PROMPT = `As a reminder, we are working to solve the following task:
...
const ORCHESTRATOR_UPDATE_PLAN_PROMPT = `Please briefly explain what went wrong on this last run (the root cause of the failure), and then come up with a new plan that takes steps and/or includes hints to overcome prior challenges and especially avoids repeating the same mistakes. As before, the new plan should be concise, be expressed in bullet-point form, and consider the following team composition (do not involve any other outside people since we cannot contact anyone else):

{team}
`;

const ORCHESTRATOR_GET_FINAL_ANSWER = `
